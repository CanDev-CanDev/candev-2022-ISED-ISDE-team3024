# -*- coding: utf-8 -*-
"""
Created on Sun Feb  6 10:27:09 2022
...
if run == 1:
    ...
    df_WORKFORCE = df.GetData_ByIndicator('WORKFORCE')
    dict_WORKFORCE = {}
    for i, group in enumerate(macro_group_list):
        dict_WORKFORCE[group] = eval('macro_{}'.format(group))
        for j,subgroup in enumerate(list(dict_WORKFORCE[group].keys())):
            targetdf = eval('df.GetData_' + group + subgroup + '()')
            outdf = create_intersection_frame(df_WORKFORCE, targetdf)
            dict_WORKFORCE[group][subgroup] = outdf
            del outdf
    ...
    WORKFORCE_was = {}
    for i, group in enumerate(macro_group_list):
        WORKFORCE_was[group] = eval('macro_{}'.format(group))
        for j,subgroup in enumerate(list(dict_WORKFORCE[group].keys())):
            staticscore = dict_WORKFORCE[group][subgroup]['SCORE100']
            staticcount = dict_WORKFORCE[group][subgroup]['ANSCOUNT']
            was = weighted_average_score(staticscore, staticcount)
            WORKFORCE_was[group][subgroup] = was
            del staticcount
            del staticscore
