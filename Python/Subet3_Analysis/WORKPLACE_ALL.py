# -*- coding: utf-8 -*-
"""
Created on Sun Feb  6 10:30:35 2022
...
if run == 1:
    ...
    print('Current Indicator is WORKPLACE.')
    print('-----------------------------------------------------')
    time.sleep(0.5)
    df_i = {}
    ...
    df_WORKPLACE = df.GetData_ByIndicator('WORKPLACE')
    dict_WORKPLACE = {}
    for i, group in enumerate(macro_group_list):
        dict_WORKPLACE[group] = eval('macro_{}'.format(group))
        for j,subgroup in enumerate(list(dict_WORKPLACE[group].keys())):
            targetdf = eval('df.GetData_' + group + subgroup + '()')
            outdf = create_intersection_frame(df_WORKPLACE, targetdf)
            dict_WORKPLACE[group][subgroup] = outdf
            del outdf
    ...
    WORKPLACE_was = {}
    for i, group in enumerate(macro_group_list):
        WORKPLACE_was[group] = eval('macro_{}'.format(group))
        for j,subgroup in enumerate(list(dict_WORKPLACE[group].keys())):
            staticscore = dict_WORKPLACE[group][subgroup]['SCORE100']
            staticcount = dict_WORKPLACE[group][subgroup]['ANSCOUNT']
            was = weighted_average_score(staticscore, staticcount)
            WORKPLACE_was[group][subgroup] = was
            del staticcount
            del staticscore
